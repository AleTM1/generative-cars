    ...
    for i in range(len(lenght_array)):
        if lenght_array[i] > 100:
            for p in range(int(len(waypoints_array[i])/2), len(waypoints_array[i])):
                if Point(waypoints_array[i][p]).distance(line) < 10:
                    print(Point(waypoints_array[i][p]).distance(line))
                    return True, i, p
    return False, -1, -1


def main_loop(actions_num, dim, sp, sa):
    def init_population(start, ang):
        return [Car(actions_num - 1, inner_poly, outer_poly, start, ang) for _ in range(dim)]

    population = init_population(sp, sa[0])
    solution = []
    j = 1
    epoch = 0
